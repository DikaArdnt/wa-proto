...
async function findAppModules() {
	...
	const versions = [...serviceworker.matchAll(/client_revision\\":([\d\.]+),/g)].map(r => r[1]);
...
(async () => {
	...
	for (const mod of modules) {
		...
		walk.simple(mod, {
			AssignmentExpression(node) {
				...
				if (left.property?.name && left.property?.name !== 'internalSpec' && left.property?.name !== 'internalDefaults') {
				...
				if (node.left.type === 'MemberExpression' && node.left.property.name === 'internalSpec' && node.right.type === 'ObjectExpression') {
					...
					members = members.map(({ key: { name }, value: { elements } }) => {
						...
						const unwrapBinaryOr = n =>
							n.type === 'BinaryExpression' && n.operator === '|' ? [].concat(unwrapBinaryOr(n.left), unwrapBinaryOr(n.right)) : [n];
	...
	for (const mod of modules) {
		const protoName = mod.expression.arguments[0].value.replace(/^(WAWebProtobufs|WAWebProtobuf|WAProtobufs|WA)/g, '').replace(/\.pb$/, '');
		const modInfo = modulesInfo[mod.expression.arguments[0].value];
		...
		for (const value of identifiers) {
			...
			if (!moduleIndentationMap[name]?.indentation?.length) {
				...
				const exists = decodedProtoMap[protoName].some(entry => entry.name === name);
				if (!exists) decodedProtoMap[protoName].push({ protoName, name, content });
			}
		}
	}
	...
	for (const protoName in decodedProtoMap) {
		...
		// Find references to types not defined in current file
		decodedProtoMap[protoName].forEach(d => {
			// Match type names in field definitions including nested types
			const typeRefs = d.content.match(/(?<=optional|required|implicit|repeated|map<)\s*([\w.]+)(?=[\s>])/g) || [];
			typeRefs.forEach(ref => {
				ref = ref.trim();
				const rootType = ref.split('.')[0];
				const protoTypes = [
					'double',
					'float',
					'int32',
				...
				if (!protoTypes.includes(rootType)) {
					// Look for the type in other proto files
					for (const otherProto in decodedProtoMap) {
						if (otherProto !== protoName) {
							const found = decodedProtoMap[otherProto].some(
								d => d.content.includes(`message ${rootType} {`) || d.content.includes(`enum ${rootType} {`)
							);
							if (found) {
								importsNeeded.add(otherProto);
								externalTypeRefs.set(ref, otherProto);
								break;
							}
						}
					}
		...
		const updatedDecodedProto = decodedProtoMap[protoName].map(d => {
			...
			externalTypeRefs.forEach((protoFile, type) => {
				const regex = new RegExp(`(optional|required|implicit|repeated|map<)\\s+(${type})(?=\\s|>)`, 'g');
